...
.randomForestClassification <- function(dataset, options, jaspResults) {
  ...
  result[["classes"]] <- predict(testFit, newdata = dataset[options[["predictors"]]])
  ...
  result[["explainer"]] <- DALEX::explain(result[["model"]], type = "multiclass", data = result[["train"]][options[["predictors"]]], y = result[["train"]][, options[["target"]]] , predict_function = function(model, data) predict(model, newdata = data, type = "prob"))
  if (nlevels(result[["testReal"]]) == 2) {
    result[["explainer_fi"]] <- DALEX::explain(result[["model"]], type = "classification", data = result[["train"]][options[["predictors"]]], y = as.numeric(result[["train"]][, options[["target"]]]) - 1, predict_function = function(model, data) predict(model, newdata = data, type = "response"))
  } else {
    result[["explainer_fi"]] <- DALEX::explain(result[["model"]], type = "multiclass", data = result[["train"]][options[["predictors"]]], y = result[["train"]][, options[["target"]]] , predict_function = function(model, data) predict(model, newdata = data, type = "prob"))
  }
  return(result)
}
