...
set.seed(1)
results <- jaspTools::runAnalysis("mlClusteringHierarchical", "iris.csv", options)

table <- results[["results"]][["clusteringTable"]][["data"]]
jaspTools::expect_equal_tables(table,
		list(3, 150, 0.669701219735835, 220.86, 256.99, 0.48))
...
test_that("Model Performance Metrics table results match", {
	table <- results[["results"]][["clusterEvaluationMetrics"]][["data"]]
	jaspTools::expect_equal_tables(table,
		list("Maximum diameter", 8.65188420870295, "Minimum separation", 1.65382813821881,
			 "Pearson's <unicode>", 0.568236148438596, "Dunn index", 0.191152366158023,
			 "Entropy", 1.63956317716101, "Calinski-Harabasz index", 39.1289637915435
			))
})
...
test_that("Cluster Information table results match", {
	table <- results[["results"]][["clusterInfoTable"]][["data"]]
	jaspTools::expect_equal_tables(table,
		list(1, 0.268080239675824, 58, 288.590093334574, 2, 0.0399824529712903,
			 6, 43.0413664531293, 3, 0.241885028860911, 30, 260.390781281077,
			 4, 0.216632828426341, 36, 233.206625935916, 5, 0.142210541161857,
			 28, 153.090557501325, 6, 0.0912089089037752, 20, 98.1869740392464
			))
})
...
test_that("Cluster Means table results match", {
	table <- results[["results"]][["clusterMeansTable"]][["data"]]
	jaspTools::expect_equal_tables(table,
		list(-0.814718951000308, 0.896746630031742, 0.188036161313956, 0.225350706617625,
			 0.747514865472435, 0.962345918273369, 0.453938607686343, 0.381452016581806,
			 -0.327160651969942, -0.620302837886437, 0.867053370206996, 0.561736591613942,
			 1.1611068462929, "Cluster 1", 1.59353832340386, 0.116258853404513,
			 1.93850225204976, -0.375172259495944, 0.988249857530258, 1.16706232279381,
			 0.929908149450233, 1.10333941218387, -0.335391806622396, -0.0684642069921506,
			 1.07568881611213, 0.48180924284713, -0.0748153887034146, "Cluster 2",
			 -0.328868448134948, -0.945954923754754, -1.12213198429158, -0.84160883941751,
			 0.458665253616639, 0.138555138801212, 0.442825108612933, -0.25496777432356,
			 -0.51233076482022, -0.692526739034442, 0.173450707350176, 0.402604855023938,
			 -0.659219851493464, "Cluster 3", 0.277664529455091, -0.177659982628153,
			 -0.00249147266452801, 0.115491991115137, -1.15888393847886,
			 -1.22789329816235, -0.793840815429072, -0.0966541933073729,
			 0.766622615430375, 0.831025064871381, -1.03787763111556, -1.09063080952508,
			 -0.389987330010673, "Cluster 4", 0.795030837821801, -1.01918778591077,
			 0.0595613112912381, -1.03072640112871, 0.239581146640469, -0.0264426995133515,
			 0.454908434690164, -0.76958415684055, -0.187907153438954, 0.633653969769103,
			 -0.292353113010822, -0.0851843527569232, -0.908317963003814,
			 "Cluster 5", 0.765086807112473, 0.530140371524552, 0.47744124800035,
			 1.95657926535688, -1.40168846358878, -1.1015268520218, -1.46759041083875,
			 0.196634357272983, 0.701028886969743, 0.475246926074739, -0.819863385236042,
			 -0.295092620065658, -0.382313118173636, "Cluster 6"))
})

test_that("Model Summary: Hierarchical Clustering table results match", {
	table <- results[["results"]][["clusteringTable"]][["data"]]
	jaspTools::expect_equal_tables(table,
		list(0.18, 1232.51, 1480.69, 6, 0.532157149697841, 178))
})
